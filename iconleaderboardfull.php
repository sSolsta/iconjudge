...
if ($result->num_rows == 0) {
	...
	$stmt = $conn->prepare("SELECT COUNT(*) AS count FROM iconScores WHERE gamemode = ?");
	$rowCount = 0;
	...
	$tableRow = array_fill(0, ($rowCount+1), "");
	for ($temp = 0; $temp <= ($rowCount); $temp++) {
		if ($temp > 0) {
			$tableRow[$temp] .= "<td>";
			$tableRow[$temp] .= "#".$temp;
			$tableRow[$temp] .= "</td>";			
	...
	foreach ($iconArray as $iconStr) {
		...
		$stmt = $conn->prepare("SELECT percentageWon, iconNumber FROM iconScores WHERE gamemode = ? ORDER BY percentageWon DESC");
		$stmt->bind_param("s", $iconStr);
		$stmt->execute();
		$result = $stmt->get_result();
		$temp = 1;
...
<p>images from gdcolon's <a href="https://gdbrowser.com">gdbrowser project</a> - <a href="iconjudge.php">judge icons yourself</a> - <a href="iconleaderboard.php">back to regular leaderboard</a> - if you see ads i didn't put them there, use an adblocker if you can</p>
